...
function changeSelect(event, name){
    var selectObj = event.target;
    addCookie(name, selectObj.value, 24*365); // 存储cookie;
}

// 生成事件用闭包
function gen_event_closure(extra){
    return function (event) {
        return changeSelect(event, extra);
    };
}
...
function processGetVer(loader){
    ...
    for (j=0;j<2;j++) {
        ...
        var defIndex = 0;
        var cookieName = escape(loader.filePath)+"_upload_"+j;
        var cookieValue = getCookie(cookieName);
        for(i=0;i<verArray.length;i++){
            if(verArray[i].length <= 1)
               continue;
            form.ver.options.add(new Option(verArray[i].split("/")[1], verArray[i]+"/"));
            if(cookieValue == verArray[i]+"/")
                defIndex = i;
        }
        
        form.ver.options.selectedIndex = defIndex;
        form.ver.onchange = gen_event_closure(cookieName);
